        ...
        if not keym.contains(user.id) and event.text != UND:
            ...
            try:
                ...
                await asst.edit_message(
                    ...
                    f"Incoming PM from **[{message.from_user.first_name}](tg://user?id={message.from_user.id})** [`{user.id}`] with **{wrn}/{WARNS}** warning!",
                    buttons=[
                ...
                _not_approved[user.id] = await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                    f"Incoming PM from **[{message.from_user.first_name}](tg://user?id={message.from_user.id})** [`{user.id}`] with **1/{WARNS}** warning!",
                    buttons=[
                        Button.inline("Approve PM", data=f"approve_{user.id}"),
                        Button.inline("Block PM", data=f"block_{user.id}"),
                    ],
